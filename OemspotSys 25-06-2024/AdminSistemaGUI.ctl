...
Begin VB.UserControl AdminSistemaGUI 
   ClientHeight    =   8685
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   12225
   ...
   Begin VB.CommandButton CmdSiguiente 
      ...
      Width           =   255
   End
   Begin VB.CommandButton CmdPropiedadesExtendidas 
      Caption         =   "Prop. Extendidas ..."
      Height          =   255
      Left            =   10560
      TabIndex        =   12
      Top             =   5040
   ...
   Begin MSComDlg.CommonDialog CommonDialog1 
      ...
      _Version        =   393216
   End
   Begin VB.CommandButton CmdCulturas 
      Caption         =   "Culturas ..."
      Height          =   255
      Left            =   10560
      TabIndex        =   9
      ...
      Width           =   1575
   End
   Begin VB.CommandButton CmdRedes 
      Caption         =   "Redes ..."
      Height          =   255
      Left            =   10560
      TabIndex        =   11
      ...
      Width           =   1575
   End
   Begin VB.CommandButton CmdArboles 
      Caption         =   "Arboles ..."
      Height          =   255
      Left            =   10560
      TabIndex        =   7
   ...
   Begin VB.Image CmdImprimir 
      Height          =   255
      Left            =   10800
      Picture         =   "AdminSistemaGUI.ctx":0000
   ...
   Begin VB.Image CmdInternetExplorer 
      Height          =   270
      Left            =   11160
      Picture         =   "AdminSistemaGUI.ctx":043E
   ...
   Begin VB.Image CmdPlanilla 
      Height          =   270
      Left            =   11520
      Picture         =   "AdminSistemaGUI.ctx":08B8
   ...
   End
End
Attribute VB_Name = "AdminSistemaGUI"
...
Sub init()
    ...
    ' FIXME: Terminar estos ABMs
    CmdEstados.Enabled = False
    CmdArboles.Enabled = False
    CmdRedes.Enabled = False
...
Sub getNumeroPaginas()
    If AdminSistemasService.getCantidadItems(txtBusqueda.Text) Then
        MaximoNroPagina = AdminSistemasService.ConfiguracionQuery.DbRecordsetQRY.Fields(0).value
...
Private Sub Combo1_Click()
    ...
    'Repaginar = False
    cmdBuscar_Click
    'If AdminSistemasService.getList(txtBusqueda.Text, (NroPagina - 1) * MAX_GRID_ROWS_SIZE_SYS) Then
...
On Error GoTo Salir
    ...
    If Repaginar Then
        ...
        getNumeroPaginas
    End If
    If AdminSistemasService.getList(txtBusqueda.Text, ((NroPagina - 1) * MAX_GRID_ROWS_SIZE_SYS) - 1) Then
        fillGrid Grid, AdminSistemasService.ConfiguracionQuery.DbRecordsetQRY, 1
    End If
    'If Repaginar Then
    '    Combo1.ListIndex = 0
...
Private Sub CmdAgregar_Click()
    Set ConfiguracionMOD.ParentWindow = Me
    ConfiguracionMOD.ConfiguracionID = 0
    ConfiguracionMOD.Show vbModal
End Sub
...
Private Sub cmdActualizar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        ...
        Grid.Row = Grid.RowSel
        Set ConfiguracionMOD.ParentWindow = Me
        ConfiguracionMOD.ConfiguracionID = CLng(Grid.Text)
        ConfiguracionMOD.Show vbModal
    End If
End Sub
...
Private Sub cmdBorrar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        If MsgBox("Está seguro de borrar [" & Grid.Text & "]?", vbYesNo, "Atención") = vbYes Then
            Grid.col = 0
            Grid.Row = Grid.RowSel
            If Not AdminSistemasService.ConfiguracionOrm.eliminarConfiguracion(CLng(Grid.Text)) Then
                MsgBox "No se puede eliminar Configuración", vbCritical, "Atención"
                Exit Sub
            End If
            UpdateWindow
        End If
...
Private Sub CmdPropiedadesExtendidas_Click()
    Unload AdminPropExtendidasMOD
    AdminPropExtendidasMOD.ReadOnly = False
    AdminPropExtendidasMOD.Seleccion = False
    Set AdminPropExtendidasMOD.ParentWindow = Nothing
    AdminPropExtendidasMOD.Show
End Sub

Private Sub CmdCategorias_Click()
    Unload AdminCategoriasMOD
    AdminCategoriasMOD.ReadOnly = False
    AdminCategoriasMOD.Seleccion = False
    Set AdminCategoriasMOD.ParentWindow = Nothing
    AdminCategoriasMOD.Show
End Sub

Private Sub CmdCulturas_Click()
    Unload AdminCulturasMOD
    Set AdminCulturasMOD.ParentWindow = Nothing
    AdminCulturasMOD.Show
End Sub
...
Private Sub CmdImprimir_Click()
    ...
    NombreImpresion = "Configuración de Sistema"
    ...
    exportarGrillaAArchivo Grid, ArchivoTemporal, "Administración de Sistema", 1
...
Private Sub CmdInternetExplorer_Click()
    ...
    If CommonDialog1.FileName <> "" Then
        exportarGrillaAArchivo Grid, CommonDialog1.FileName, "Administración de Sistema", 1
    End If
End Sub
...
Sub actualizarLocalizacion()
    ...
    CmdArboles.Caption = localizar(CmdArboles.name)
    CmdCategorias.Caption = localizar(CmdCategorias.name)
    CmdCulturas.Caption = localizar(CmdCulturas.name)
    CmdEstados.Caption = localizar(CmdEstados.name)
    CmdRedes.Caption = localizar(CmdRedes.name)
    CmdPropiedadesExtendidas.Caption = localizar(CmdPropiedadesExtendidas.name)
    lblSeleccion.Caption = localizar(lblSeleccion.name)
End Sub
