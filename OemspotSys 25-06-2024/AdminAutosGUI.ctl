...
Begin VB.UserControl AdminAutosGUI 
   ClientHeight    =   8655
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   12270
   ...
   Begin VB.Image CmdImprimir 
      Height          =   255
      Left            =   10920
      Picture         =   "AdminAutosGUI.ctx":0000
   ...
   Begin VB.Image CmdInternetExplorer 
      Height          =   270
      Left            =   11280
      Picture         =   "AdminAutosGUI.ctx":043E
   ...
   Begin VB.Image CmdPlanilla 
      Height          =   270
      Left            =   11640
      Picture         =   "AdminAutosGUI.ctx":08B8
   ...
   End
End
Attribute VB_Name = "AdminAutosGUI"
...
Private flag_llenando As Boolean
...
On Error GoTo Salir
    ...
    If AdminAutosService.getCantidadItems(txtBusqueda.Text) Then
        MaximoNroPagina = AdminAutosService.AutosQuery.DbRecordsetQRY.Fields(0).value
...
'    If AdminAutosService.getList(txtBusqueda.Text, (NroPagina - 1) * MAX_GRID_ROWS_SIZE_SYS) Then
'        fillGrid Grid, AdminAutosService.AutosQuery.DbRecordsetQRY, 4
'    End If
   cmdBuscar_Click
Exit Sub
Salir:
MsgBox "AdminAutosGUI.cmdSiguiente_Click" & vbCrLf & _
...
On Error GoTo Salir
    ...
    If flag_llenando Then Exit Sub
...
Salir:
Descargar_Cartel
MsgBox "AdminAutosGUI.Combo1_Click" & vbCrLf & _
           "Descripción: " & Err.Description & vbCrLf & _
           "Número error: " & Err.Number, _
           vbCritical, _
           "Error en Ejecución"
...
On Error GoTo Salir
    ...
    If AdminAutosService.getCantidadItems(txtBusqueda.Text) Then
    'lblCantidadDeRegistros.Caption = CLng(AdminAutosService.AutosOrm.Fields(0).value) & " Registros."
    MaximoNroPagina = AdminAutosService.AutosQuery.DbRecordsetQRY.Fields(0).value
...
Salir:
       Descargar_Cartel
           MsgBox "AdminAutosGUI.Buscar" & vbCrLf & _
           "Descripción: " & Err.Description & vbCrLf & _
           "Número error: " & Err.Number, _
           vbCritical, _
           "Error en Ejecución"
...
Private Sub CmdAgregar_Click()
    Set AutoMOD.ParentWindow = Me
    AutoMOD.AutoID = 0
    AutoMOD.Show vbModal
End Sub
...
Private Sub cmdBorrar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        If MsgBox("Está seguro de borrar [" & Grid.Text & "]?", vbYesNo, "Atención") = vbYes Then
            Grid.col = 0
            Grid.Row = Grid.RowSel
            If Not AdminAutosService.AutosOrm.eliminarAuto(CLng(Grid.Text)) Then
...
Private Sub CmdImprimir_Click()
    ...
    exportarGrillaAArchivo Grid, ArchivoTemporal, "Administración de Autos", 1
...
Private Sub CmdInternetExplorer_Click()
    ...
    If CommonDialog1.FileName <> "" Then
        exportarGrillaAArchivo Grid, CommonDialog1.FileName, "Administración de Autos", 1
    End If
End Sub
...
Exit Sub
Salir:
    MsgBox "AdminAutosGUI.Excel" & vbCrLf & _
           "Descripción: " & Err.Description & vbCrLf & _
           "Número error: " & Err.Number, _
           vbCritical, _
           "Error en Ejecución"
...
On Error GoTo Salir
 ...
 Call Separar_en_Vector(txtBusqueda.Text, V)
    ...
    For i = 1 To MaximoNroPagina
        If AdminAutosService.getList(txtBusqueda.Text, (i - 1) * MAX_GRID_ROWS_SIZE_SYS) Then
            If i = 1 Then
                exportarQueryAArchivo AdminAutosService.AutosQuery.DbRecordsetQRY, NombreArchivo, "Reporte de Autos", 4, True
            Else
                exportarQueryAArchivo AdminAutosService.AutosQuery.DbRecordsetQRY, NombreArchivo, "Reporte de Autos", 4, False
            End If
        End If
...
Exit Sub
Salir:
    MsgBox "AdminAutosGUI.Exportar" & vbCrLf & _
           "Descripción: " & Err.Description & vbCrLf & _
           "Número error: " & Err.Number, _
           vbCritical, _
           "Error en Ejecución"
