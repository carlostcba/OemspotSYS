...
Begin VB.UserControl AdminUsuariosGUI 
   ClientHeight    =   8685
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   12225
   ...
   Begin VB.Image CmdPlanilla 
      Height          =   270
      Left            =   11520
      Picture         =   "AdminUsuariosGUI.ctx":0000
   ...
   Begin VB.Image CmdInternetExplorer 
      Height          =   270
      Left            =   11160
      Picture         =   "AdminUsuariosGUI.ctx":0432
   ...
   Begin VB.Image CmdImprimir 
      Height          =   255
      Left            =   10800
      Picture         =   "AdminUsuariosGUI.ctx":08AC
   ...
   End
End
Attribute VB_Name = "AdminUsuariosGUI"
...
Sub getNumeroPaginas()
    If AdminUsuariosService.getCantidadItems(txtBusqueda.Text) Then
        MaximoNroPagina = AdminUsuariosService.UsuarioQuery.DbRecordsetQRY.Fields(0).value
...
Private Sub Combo1_Click()
    ...
    cmdBuscar_Click
    If AdminUsuariosService.getList(txtBusqueda.Text, (NroPagina - 1) * MAX_GRID_ROWS_SIZE_SYS) Then
...
Private Sub CmdAgregar_Click()
    If AdminUsuariosService.tieneLicencias Then
        Set UsuarioMOD.ParentWindow = Me
        UsuarioMOD.UsuarioID = 0
        UsuarioMOD.Show vbModal
    Else
        MsgBox "No tiene Licencia para agregar más Usuarios al Servidor VIDEOMAN. Por favor, haga click en la opción de menú Principal->Extender Sistema, para solicitar la extensión del Servidor", vbCritical, "Servidor VIDEOMAN"
    End If
End Sub
...
Private Sub cmdActualizar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        ...
        Grid.Row = Grid.RowSel
        Set UsuarioMOD.ParentWindow = Me
        UsuarioMOD.UsuarioID = CLng(Grid.Text)
        UsuarioMOD.Show vbModal
    End If
End Sub
...
Private Sub cmdBorrar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        If MsgBox("Está seguro de borrar [" & Grid.Text & "]?", vbYesNo, "Atención") = vbYes Then
            Grid.col = 0
            Grid.Row = Grid.RowSel
            If Not AdminUsuariosService.UsuarioOrm.eliminarUsuario(CLng(Grid.Text)) Then
                MsgBox "No se puede eliminar Usuario", vbCritical, "Atención"
                Exit Sub
            End If
            UpdateWindow
        End If
...
Private Sub CmdImprimir_Click()
    ...
    exportarGrillaAArchivo Grid, ArchivoTemporal, "Administración de Usuarios", 1
...
Private Sub CmdInternetExplorer_Click()
    ...
    If CommonDialog1.FileName <> "" Then
        exportarGrillaAArchivo Grid, CommonDialog1.FileName, "Administración de Usuarios", 1
    End If
End Sub
