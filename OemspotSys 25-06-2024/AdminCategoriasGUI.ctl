...
Begin VB.UserControl AdminCategoriasGUI 
   ClientHeight    =   8670
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   12135
   ...
   Begin VB.Image CmdPlanilla 
      Height          =   270
      Left            =   11520
      Picture         =   "AdminCategoriasGUI.ctx":0000
   ...
   Begin VB.Image CmdInternetExplorer 
      Height          =   270
      Left            =   11160
      Picture         =   "AdminCategoriasGUI.ctx":0432
   ...
   Begin VB.Image CmdImprimir 
      Height          =   255
      Left            =   10800
      Picture         =   "AdminCategoriasGUI.ctx":08AC
   ...
   End
End
Attribute VB_Name = "AdminCategoriasGUI"
...
Sub getNumeroPaginas()
    If AdminCategoriasService.getCantidadItems(txtBusqueda.Text) Then
        MaximoNroPagina = AdminCategoriasService.CategoriasQuery.DbRecordsetQRY.Fields(0).value
...
Private Sub Combo1_Click()
 ...
 '   cmdBuscar_Click
    If AdminCategoriasService.getList(txtBusqueda.Text, (NroPagina - 1) * MAX_GRID_ROWS_SIZE_SYS) Then
...
Private Sub CmdAgregar_Click()
    Set CategoriaMOD.ParentWindow = Me
    CategoriaMOD.CategoriaID = 0
    CategoriaMOD.Show vbModal
End Sub
...
Private Sub cmdActualizar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        ...
        Grid.Row = Grid.RowSel
        Set CategoriaMOD.ParentWindow = Me
        CategoriaMOD.CategoriaID = CLng(Grid.Text)
        CategoriaMOD.Show vbModal
    End If
End Sub
...
Private Sub cmdBorrar_Click()
    ...
    If Grid.Text <> "" And Grid.Row >= 1 Then
        If MsgBox("Está seguro de borrar [" & Grid.Text & "]?", vbYesNo, "Atención") = vbYes Then
            Grid.col = 0
            Grid.Row = Grid.RowSel
            If Not AdminCategoriasService.CategoriasOrm.eliminarCategoria(CLng(Grid.Text)) Then
                MsgBox "No se puede eliminar Categoría", vbCritical, "Atención"
                Exit Sub
            End If
            UpdateWindow
        End If
...
Private Sub CmdImprimir_Click()
    ...
    exportarGrillaAArchivo Grid, ArchivoTemporal, "Administración de Categoriaes Extendidas", 1
...
Private Sub CmdInternetExplorer_Click()
    ...
    If CommonDialog1.FileName <> "" Then
        exportarGrillaAArchivo Grid, CommonDialog1.FileName, "Administración de Categorías", 1
    End If
End Sub
